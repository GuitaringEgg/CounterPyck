...
opencv_haystack =cv2.imread('ss.png')
opencv_needle =cv2.imread('images/lina.png')
opencv_template = cv2.imread('images/zeus.png')

ngrey = cv2.cvtColor(opencv_needle, cv2.COLOR_BGR2GRAY)
hgrey = cv2.cvtColor(opencv_haystack, cv2.COLOR_BGR2GRAY)
tgrey = cv2.cvtColor(opencv_template, cv2.COLOR_BGR2GRAY)
...
(tkeypoints, tdescriptors) = detector.detectAndCompute(tgrey, None, useProvidedKeypoints = False)
...
# retrieve index and value through enumeration
for i, descriptor in enumerate(trows):
    descriptor = numpy.array(descriptor, dtype = numpy.float32).reshape((1, rowsize))
    #print i, descriptor.shape, samples[0].shape
    retval, results, neigh_resp, dists = knn.find_nearest(descriptor, 1)
    res, dist =  int(results[0][0]), dists[0][0]
    ...
    x,y = tkeypoints[i].pt
    center = (int(x),int(y))
    cv2.circle(opencv_template,center,2,color,-1)
